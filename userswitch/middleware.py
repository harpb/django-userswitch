...
class UserSwitchMiddleware(MiddlewareMixin):
    def __init__(self, get_response=None):
        ...
        self.USERSWITCH_OPTIONS = {
            ...
            'select_css_class': settings.USERSWITCH_OPTIONS.get('select_css_class', 'form-control'),
            'css_inline': settings.USERSWITCH_OPTIONS.get('css_inline',
                'position:absolute;bottom:5px;right:5px;z-index:16777271;'),
            'content_types': settings.USERSWITCH_OPTIONS.get('content_types', ('text/html', 'application/xhtml+xml')),
            'auth_backend': settings.USERSWITCH_OPTIONS.get('auth_backend',
        ...
        <select class="%(select_css_class)s"  onChange="var username = options[selectedIndex].value; document.location.href = '/?userswitch_username=' + encodeURIComponent(username);">
            <options>
        </select>
        </div>
        """ % self.USERSWITCH_OPTIONS
    ...
    def process_response(self, request, response):
        ...
        """
        if response.status_code == 200 and response['Content-Type'].split(';')[0].strip() in self.USERSWITCH_OPTIONS[
            'content_types']:
            ...
            if self.USERSWITCH_OPTIONS['replace_text']:
                response.content = html.replace(self.USERSWITCH_OPTIONS['replace_text'], switch_widget)
            else:
                response.content = html.replace('</body>', '{}</body>'.format(switch_widget))
